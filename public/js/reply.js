  ...
  $('.js-replist').click(function () {
    ...
    $.ajax({
      ...
      $.each(data, function (index, value) {
        ...
        var name = value.name;
        var comment_serial = value.comment_serial;
        var body = value.body;
        var like_count = value.like_count;
        var dislike_count = value.dislike_count; // 日付

        var created_at = value.created_at;
        var date = new Date(created_at); // let year = date.getFullYear();

        var month = ('00' + (date.getMonth() + 1)).slice(-2);
        var day = ('00' + date.getDate()).slice(-2);
        var hour = date.getHours();
        var minute = date.getMinutes();
        var viewdate = month + '/' + day + '\t' + hour + ':' + minute; // <button class="dislike-toggle" data-review-id="${id}"><i class="fa-solid fa-thumbs-down relative top-1 mr-1 text-md"></i><span class="dislike-counter text-md">${dislike_count}</span></button>

        html += "\n                        <div class=\"border-b px-5 pb-2 pt-2 {{$comment->user_id}}\">\n                        <div class=\"flex justify-between text-xs text-gray-500 mb-3\">\n                        <div class=\"\">\n                        <span>".concat(comment_serial, "</span>\n                        <span>").concat(name, "</span>&nbsp;").concat(viewdate, " ID:").concat(user_id, "&nbsp;\n                        </div>\n                        </div>\n                        <div class=\"thcombody font-normal text-md text-gray-700 mb-1 mt-3\">").concat(body, "</div>   \n                        <div class=\"py-4 mb-1 flex text-sm\">  \n                        <div class=\"flex text-gray-400 items-center mr-3\">\n                        <button class=\"like-toggle mr-2\" data-review-id=\"").concat(id, "\"><i class=\"fa-solid fa-thumbs-up text-md mr-1\"></i><span class=\"like-counter text-md mr-1\">").concat(like_count, "</span></button>\n                         \n                        </div>\n                        <button class=\"js-repform text-gray-400\" data-repserial=\"").concat(comment_serial, "\" data-repid=\"").concat(id, "\"><i class=\"fa-solid fa-reply\"></i></button>\n                        </div>\n                        </div>\n\n                    ");
      });
...
$(function () {
  $('body').on('click', '.reppop', function () {
    ...
    id = $this.parent('.thcombody').data('commentid'); // 位置
    ...
    $.ajax({
      ...
      var name = data.name;
      var comment_serial = data.comment_serial;
      var body = data.body; // 日付
      ...
      var hour = date.getHours();
      var minute = date.getMinutes();
      var viewdate = month + '/' + day + '\t' + hour + ':' + minute; // html = `
